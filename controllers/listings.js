...
async function geocodeLocation(location) {
  const geometry = {
    type: "Point",
    coordinates: [77.209, 28.6139] // Default coordinates (New Delhi, India)
  };
  if (!location) return geometry;
  ...
  try {
    ...
    console.error("MapTiler Geocoding failed, using default coordinates:", err);
  }
  return geometry;
}
...
module.exports.updateListing = async (req, res, next) => {
  let { id } = req.params;
  let geometry = await geocodeLocation(`${req.body.listing.location}, ${req.body.listing.country}`);
